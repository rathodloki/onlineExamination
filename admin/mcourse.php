    ...
    <div id="site_content" style="background-image: none">
      <div id="content" style=" width: 1000px; height: auto;min-height:590px">
               ...
               <div class="manager">
                       ...
                       <div class="hover"><div class="head">Course name</div><div class="head">Subjects</div><div class="head">Edit</div><div class="head">Delete</div></div>
                      
                      <?php
              $cone=mysqli_connect("localhost","root","","exam") or die(mysql_errno());
              mysqli_select_db($cone,"exam") or die("cannot connect to database");
              
              $query2=mysqli_query($cone,"select * from courses");

                while ($tables=mysqli_fetch_array($query2))
                {
                    #special char to string portable
                 // $query2=mysqli`_query($cone,"select cname from courses where c_id=".$tables[1]);
                 // $course=mysqli_fetch_array($query2);
  
                   print'<div class="hover_r"><div class="rows">'.$tables['cname'].'</div>';
                   $query1=mysqli_query($cone,"select s_name from subject where c_id=".$tables['c_id']." ");
                    print'<div class="rows"><select class="inadd" style="min-width:10px;">';
                   while($Subject_name=mysqli_fetch_array($query1))
                   {
                   print'<option>'.$Subject_name['s_name'].'</option>';
                    }
                    print'</select></div>';
                   print'<a href="editcourse.php?osubject='.urlencode($tables['cname']).'" id="'.$tables['c_id'].'" class="btnanchor btn-primary btn-xs rows-e"><i style=" font-size:15px;" class="fa fa-edit"></i> </a><a href="delcourse.php?osubject='.urlencode($tables['c_id']).'"   id="'.$tables['c_id'].'" class="rows-d btn btnanchor btn-danger btn-xs"> <i style=" font-size:15px;" class="fa fa-trash"></i></a></div>';
                }
              ...
              ?>
                 ...
                 <input class="badd" type="button" onclick="location.href='addcourse.php';" value="Add Course" />
               </div>
    </div>
  </div>
    <div id="footer">
